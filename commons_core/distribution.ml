...
let worker ~fmap:map_ex = 
  ...
  (*s: debug worker *)
  if !debug_mpi 
  then Common2._prefix_pr := (spf "W%d:" rank);
  if !debug_mpi
...
let master ?(timeout=60) ~freduce:reduce_ex acc xs = 
  ...
  try (
    ...
    for i = 1 to actual_workers do
      let arg = Common2.pop2 in_list in
      Mpi.send (DataIn arg) i notag Mpi.comm_world;
      incr working;
    done;
    (*e: send initial work to valid workers *)
    (*s: kill idle workers *)
    killing_workers (Common2.enum_safe (actual_workers+1) available_workers);
    (*e: kill idle workers *)
    ...
    killing_workers (Common2.enum 1 actual_workers);
    flush stderr;flush stdout;
  ...
  with e ->
    ...
    | Timeout -> 
        ...
        pr2 (spf "!!!!PB: mpi master timeout. Done %d/%d!!!!"
                total_done total);
        killing_workers (Common2.enum 1 available_workers); 

        let not_done = 
          xs |> List.filter (fun x -> not (Hashtbl.mem in_done x))
        in
        
        if Common2.pourcent total_done total > 95
        then !acc, not_done
        else begin 
...
let mpi_adjust_argv argvold = 
  ...
  then 
    ...
    begin
      ...
      let xs = xs |> Common2.take_until (fun s -> s = "-p4pg") in
